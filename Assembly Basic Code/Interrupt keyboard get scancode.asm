...
prn_msg:
	addi 	$v0, $zero, 4
	la 	$a0, Message # print text "Key scan code "
	syscall
get_cod:
        li 	$t1, IN_ADRESS_HEXA_KEYBOARD
	li 	$t3, 0x81 	# check row 1 and re-enable bit 7 => vi bit 7 là 0x80, bit 1 là 0x01 thi la 0x81
	sb 	$t3, 0($t1) 	# must reassign expected row de check in_address check row1
	li 	$t1, OUT_ADRESS_HEXA_KEYBOARD # => xem out_address ?e biet co phai no vua dc bam
	lb 	$a0, 0($t1) # => lay gia tri scan code bam dc
	bne 	$a0, $zero, prn_cod # => in ra gia tri scan code
	...
	# neu interrupt kp do bam phim tao ra thi van ch?y xuong duoi in ra rong
